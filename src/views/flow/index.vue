<template>
  <div id="flow"></div>
</template>

<script setup lang="ts">
import LogicFlow from '@logicflow/core'; // 引入 LogicFlow 实例
// 从logicflow组件包中解构出控制面板、右键菜单、拖拽面板
import {
  Control,
  Menu,
  DndPanel,
  BpmnElement,
  Snapshot,
} from '@logicflow/extension';
import '@logicflow/core/dist/style/index.css'; // 引入 LogicFlow 样式
import '@logicflow/extension/lib/style/index.css'; // 引入 LogicFlow 组件样式
import UserTask from './UserTaskNode.js'; // 自定义 用户任务 组件

LogicFlow.use(Control); // 控制面板
LogicFlow.use(Menu); // 右键菜单
LogicFlow.use(DndPanel); // 拖拽面板
LogicFlow.use(BpmnElement); // BNPM
LogicFlow.use(Snapshot); // 导出为图片

onMounted(() => {
  initFlow();
});

// 初始化图表配置
const initFlow = () => {
  // logicflow 配置项
  const lf = new LogicFlow({
    container: document.getElementById('flow') as HTMLElement, // 容器
    height: 600, // 画布高度
    width: 1200, // 画布宽度
    keyboard: {
      enabled: true,
    }, // 开启默认快捷键，如：Ctrl+c复制，Ctrl+v粘贴，Ctrl+z撤销
    grid: true, // 背景显示网格
  });
  // 拖拽面板内容
  lf.extension.dndPanel.setPatternItems([
    {
      type: 'bpmn:startEvent',
      text: '开始',
      label: '开始节点',
      icon: 'data:image/png;base64,',
    },
    {
      type: 'bpmn:userTask',
      label: '用户任务',
      text: '用户任务',
      icon: 'data:image/png;base64,',
      className: 'important-node',
    },
    {
      type: 'bpmn:serviceTask',
      label: '系统任务',
      icon: 'data:image/png;base64,',
      className: 'import_icon',
    },
    {
      type: 'diamond',
      label: '条件判断',
      icon: 'data:image/png;base64,',
    },
    {
      type: 'bpmn:exclusiveGateway',
      label: '网关',
      icon: 'data:image/png;base64,',
    },
    {
      type: 'bpmn:endEvent',
      text: '结束',
      label: '结束节点',
      icon: 'data:image/png;base64,',
    },

    {
      type: 'rect',
      label: '矩形',
      icon: 'data:image/png;base64,',
      className: 'important-node',
    },
    {
      type: 'ellipse',
      label: '椭圆',
      icon: 'data:image/png;base64,',
      className: 'important-node',
    },
    {
      type: 'polygon',
      label: '多边形',
      icon: 'data:image/png;base64,',
      className: 'important-node',
    },
  ]);
  lf.register(UserTask);
  lf.render(); // 渲染画布
};
</script>

<style scoped></style>
